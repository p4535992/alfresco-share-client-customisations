...
<div class="yui-g wikipage-bar">
  <div <#if action == "view" || action="details">class='eslPMWiki${eslPM?replace(" ","")}<#else>class="ESLInvisible"</#if>'>
...
<#-- We process the deletionState and deletionAuthorisation logic in two seperate if-then blocks, which will involve making some elements invisible twice, but makes for more readable code-->
<#if result.deletionState?? && result.deletionState!="deleted">
  <#if result.archiveDue??>
   <div class="deleteMarkWrapper wikiDeleteMarkWrapper">
   <@displayDeleteDate result.archiveDue />
   <@displayDeleteMark result.deletedBy result.title />
   <script language='javascript'>
     YAHOO.util.Selector.query('div.undelete')[0].style.cssText="display:none";
     YAHOO.util.Selector.query('div.mark-page-for-delete')[0].style.cssText="display:none";
   </script>
   </div>
   <div class="clearLeft"></div>
  <#else>
   <script language='javascript'>
	YAHOO.util.Selector.query('div.remove-delete-mark')[0].style.cssText="display:none";
	YAHOO.util.Selector.query('div.undelete')[0].style.cssText="display:none";
	YAHOO.util.Selector.query('div.delete-now')[0].style.cssText="display:none";
	<#if result.isDeletable == "no" || !result.pagetext??> <#-- rel INT-442, pages without pagetext are stub pages and shouldn't have delete buttons rendered against them-->
	  YAHOO.util.Selector.query('div.mark-page-for-delete')[0].style.cssText="display:none";
	</#if>
   </script>
...
<#-- Normal users and admins can't undelete or delete now, only Deleters-->
<#if !result.deletionAuthorisation?? || result.deletionAuthorisation=="normal">
   <script language='javascript'>
     YAHOO.util.Selector.query('div.undelete')[0].style.cssText="display:none";
     YAHOO.util.Selector.query('div.delete-now')[0].style.cssText="display:none";
     <#if result.isDeletable=="no">
      YAHOO.util.Selector.query('div.mark-page-for-delete')[0].style.cssText="display:none";
     </#if>
   </script>
</#if>
