...
(function()
{
   ...
   */
   ...
   Alfresco.module.DoclibPermissionsBasic = function(htmlId)
   {
      Alfresco.module.DoclibPermissionsBasic.superclass.constructor.call(this, "Alfresco.module.DoclibPermissionsBasic", htmlId, ["button", "container", "connection", "json"]);
   ...
   YAHOO.extend(Alfresco.module.DoclibPermissionsBasic, Alfresco.component.Base,
   {
      /**
      ...
      showDialog: function DLP_showDialog()
      {
         ...
         if (!this.containerDiv)
         {
            ...
            {
               url: Alfresco.constants.URL_SERVICECONTEXT + "modules/documentlibrary/permissions-basic",
               dataObj:
               {
                  htmlid: this.id,
      ...
      onTemplateLoaded: function DLP_onTemplateLoaded(response)
      {
         ...
         var roleElementId = this.id + "-permission"; 
         
         this.widgets.rolePicker = new YAHOO.widget.Button(roleElementId,
         {
            type: "menu", 
            menu: roleElementId + "-select"
         });
         this.widgets.rolePicker.getMenu().subscribe("click", this.onRoleSelected, this.widgets.rolePicker);
      ...
      _showDialog: function DLP__showDialog()
      {
         ...
         if(this.widgets.rolePicker) {
	         this.widgets.rolePicker.set("name", "");
	         this.widgets.rolePicker.set("label", this.msg("role.None"));
         }
         ...
         if(this.widgets.rolePicker) {
	         ...
	         for (i = 0, j = defaultRoles.length; i < j; i++)
	         {
	            ...
	            if (/SiteCollaborator$/.test(permissions[1]) && (permissions[2] in this.options.roles))
	            {
	               this.widgets.rolePicker.set("name", permissions[2]);
	               this.widgets.rolePicker.set("label", this.msg("role." + permissions[2]));
	            }
	         }
         }
      ...
      _parseUI: function DLP__parseUI()
      {
         ...
         role = this.widgets.rolePicker.get("name");
         
         if(role) {
        	 for(var group in this.options.groups) {
        		 if(group == "EVERYONE") {
        			 continue;
        		 }
                 ...
                 {
	        		 params.push({
	        			 group: this.options.groups[group],
	        			 role: role
	        		 });
                 }
        	 }
   ...
   var dummyInstance = new Alfresco.module.DoclibPermissionsBasic("null");
})();
