...
Alfresco.util.combinePaths = function()
{
   ...
   return path.replace(/(^|[^:])\/{2,}/g, "$1/").replace(/(.)\/$/g, "$1");
};
...
Alfresco.util.getTags = function(str)
{
   ...
   while (match = regexp.exec(tagName))
   {
      tag = match[1] || match[2];
...
//Append version query paramter to avatar URL to prevent caching in IE8
function preventAvatarCache() {
	...
	Alfresco.util.YUILoaderHelper.require(["cookie"], function(){
		...
		if(avatarVersion == undefined || avatarVersion == null) {
			// Do nothing as no cookie has been set.
			return true;
		}
		
		// Select current user's avatars from DOM
		var avatars = YAHOO.util.Selector.query('img[src*="share/proxy/alfresco/sv-theme/user-profile/avatar?user='+Alfresco.constants.USERNAME+'"]');
		
		// Append version value to all of current users avatars to avoid caching, if not already set
		for (var avatar, i = avatars.length; i-- && (avatar = avatars[i]);) {
			
			var src = YAHOO.util.Dom.getAttribute(avatar, "src");
			
			if(src.indexOf('&v=') == -1) {
				YAHOO.util.Dom.setAttribute(avatar, "src", src+"&v="+avatarVersion);
			}
...
//Setup when avatar cache prevention should run
function schedulePreventAvatarCache() {
