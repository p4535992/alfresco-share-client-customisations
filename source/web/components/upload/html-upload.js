...
(function()
{
   ...
   YAHOO.extend(Alfresco.HtmlUpload, Alfresco.component.Base,
   {
      ...
      onReady: function HtmlUpload_onReady()
      {
         ...
         this.widgets.perishableRadioButtons = YAHOO.util.Selector.query("input[name='perishable']", this.id + "-dialog");
         
         for(var i = 0; i < this.widgets.perishableRadioButtons.length; ++i) {
             form.addValidation(this.widgets.perishableRadioButtons[i], SurevineValidators.mandatory, null, "click");
         }
      ...
      show: function HtmlUpload_show(config)
      {
    	 ...
    	 {    		 
    		 ...
    		 var radios = YAHOO.util.Selector.query("input", this.id + "-perishability");
