...
(function()
{
   ...
   YAHOO.extend(Alfresco.FlashUpload, Alfresco.component.Base,
   {
      ...
      eslPM: null,
      eslOpenGroups: null,
      eslClosedGroups: null,
      eslOrganisations: null,
      eslFreeFormCaveats: null,
      eslEyes: null,
      
      /**
       * Nodelist of the perishable radio buttons
       */
      perishableRadioButtons: null,
      ...
      onReady: function FlashUpload_onReady()
      {
         ...
         this.perishableRadioButtons = YAHOO.util.Selector.query("input[name='perishable']", this.id + "-dialog");

         for(var i = 0; i < this.perishableRadioButtons.length; ++i) {
        	 YAHOO.util.Event.addListener(this.perishableRadioButtons[i], "click", this.validate, this, true);
         }
      ...
      onUploadError: function FlashUpload_onUploadError(event)
      {
	    ...
	    //}
        //    else {
	    //	fileInfo.state=this.STATE_RETRY;
        //        this.uploader.cancel(event.id);
	    //}
	 ...
	 fileInfo.progressPercentage.innerHTML = "100%";}
      ...
      validate: function() {
    	  ...
    	  if(this.state != this.STATE_BROWSING) {
    		  valid = false;
    	  } else if(this.dataTable.getRecordSet().getLength() === 0) {
    		  valid = false;
    	  }
    	  
    	  if((this.perishableRadioButtons.length > 1) && !this.showConfig.updateNodeRef) {
    		  for(var i = 0; i < this.perishableRadioButtons.length; ++i) {
    			  var oneChecked = false;
    			  
    			  if(this.perishableRadioButtons[i].checked) {
    				  oneChecked = true;
    				  break;
    			  }
    		  }
    	  ...
    	  this.widgets.uploadButton.set("disabled", !valid);
      },
      ...
      _applyConfig: function FlashUpload__applyConfig()
      {
    	 ...
    	 { 
    		 ...
    		 Dom.addClass(this.id + "-perishability", "hidden")
    	 }
    	 else
         {
    		 Dom.get(this.id + "-tag-picker-currentValueDisplay").innerHTML="Loading...";
    		 ...
    		 Dom.removeClass(this.id + "-perishability", "hidden")
         }
      ...
      _uploadFromQueue: function FlashUpload__uploadFromQueue(noOfUploadsToStart)
      {
         ...
         for (var i = 0; i < length && startedUploads < noOfUploadsToStart; i++)
         {
            ...
            if (fileInfo.state === this.STATE_BROWSING || fileInfo.state === this.STATE_RETRY)
            {
               ...
               // Only update perishability if we're uploading a new file
               if(!this.showConfig.updateNodeRef) {
            	   attributes.perishable=Dom.getElementsBy(function (el) { return (el.name === "perishable" && el.checked === true); }, 'input', 'perishable-select', null, null, null, true).value || "";
               }
