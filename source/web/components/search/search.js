   ...
   YAHOO.extend(Alfresco.Search, Alfresco.component.Base,
   {
      ...
      _setupDataTable: function Search_setupDataTable()
      {
         ...
         renderCellThumbnail = function Search_renderCellThumbnail(elCell, oRecord, oColumn, oData)
         {
            ...
            switch (dataType)
            {
               ...
               case "person":
            	  imageUrl = Alfresco.constants.PROXY_URI_RELATIVE+"sv-theme/user-profile/avatar?user="+oRecord.getData("userName");
                  break;
            }
         ...
         renderCellDescription = function Search_renderCellDescription(elCell, oRecord, oColumn, oData)
         {
            ...
            var uid = oRecord.getData("nodeRef").replace("workspace://SpacesStore/","");
            
            var desc = '';
            
            if( oRecord.getData("isMarkedForDelete") === 'yes' || oRecord.getData("isMarkedAsPerishable") === 'yes' ) {
            	
            	desc += '<div class="deleteMarkWrapper searchDeleteMarkWrapper">';
            	
            	var archivalStatus = oRecord.getData("archivalStatus");
            	
            	if(archivalStatus && archivalStatus.archivalDue) {
              	   desc += "<div class='deleteDate'>Due for deletion on <span class='date'>"+archivalStatus.archivalDue+"</span></div>";
            	}
            	
                if( 'yes' === oRecord.getData("isMarkedForDelete") ) {                   
                	desc += '<div class="markedForDeletion" id="DeletionMark-'+uid+'">Marked for deletion <span class="markMeta">by <a href="/share/page/user/'+oRecord.getData("markedForDeleteBy")+'/profile">'+oRecord.getData("markedForDeleteBy")+'</a></span></div>';
                }

                if( 'yes' === oRecord.getData("isMarkedAsPerishable") ) {
               	 	var perishLabel = me.msg("label.markedAsPerishable", oRecord.getData("perishTitle"), oRecord.getData("perishRequestedBy"));
                	desc += '<div id="PerishableMark-'+uid+'" class="markedAsPerishable">'+perishLabel+' <span class="markMeta">by <a href="/share/page/user/'+oRecord.getData("perishRequestedBy")+'/profile">'+oRecord.getData("perishRequestedBy")+'</a></span></div>';
                }
                
                desc += '</div><div class="clearLeft"></div>';
            	
            }
            
            desc += '<h3 class="itemname"><a href="' + url + '" class="theme-color-1">' + $html(displayName) + '</a>';
            ...
            switch (type)
            {
               ...
               case "person": //We don't need to label people (except via alt text) - it's obvious what they are
            	   break;
            ...
            if (type=="person") //People have fundamentally different data to cm:content nodes
            {
            	var bio = oRecord.getData("biography");
            	if (bio!=null)
            	{
	            	bio = bio.replace(/\s/g, ' '); //replace all whitespace with spaces
	            	if (bio.length>100)
	            	{
	            		bio=bio.substr(0,96)+"...";
	            	}
      ...
      _getBrowseUrlForRecord: function Search__getBrowseUrlForRecord(record)
      {
         ...
         switch (type)
         {
            ...
            case "person":
        	{
        		url = "user/" + record.getData("userName") + "/profile"
        		break;
        	}
         }
