...
(function()
{
   ...
   YAHOO.extend(Alfresco.ObjectFinder, Alfresco.component.Base,
   {
      ...
      onReady: function ObjectFinder_onReady()
      {
         ...
         /**
          * If we're being used as a tag picker, register event listeners on the hidden text box
          */
         var tagPickerCurrentField = Dom.get(this.id+"-currentValueField");
         if (tagPickerCurrentField!=null)
         {
        	 YAHOO.util.Event.addListener(tagPickerCurrentField, "change", this.specifyCurrentTags(tagPickerCurrentField.value));
         }
      ...
      /**
       * @param tags Space-seperated list of tag names
       */
      specifyCurrentTags: function ObjectFinder_specifyCurrentTags(tags)
      {
    	  var onSuccess = function ObjectFinder_specifyCurrentTags_onSuccess(response)
          {
             ...
             {
                 this.options.currentValue += tags[i].nodeRef;
                 if (i < tags.length-1)
                 {
                    this.options.currentValue += ","; 
                 }
          ...
          var onFailure = function ObjectFinder_specifyCurrentTags_onFailure(response)
          {
         ...
         {
            url: Alfresco.constants.PROXY_URI + "svtheme/tags/nodeRefs?tagNames="+YAHOO.lang.trim(tags).replace(/ /g,","),
            method: "GET",
            successCallback:
            {
            	fn: onSuccess,
         ...
         if ($hasEventInterest(this, args))
         {
            ...
            else
            {
               ...
               if (this.options.displayMode == "items")
               {
                  ...
                  var field = Dom.get(this.id+"-currentValueField");
                  if (field!=null)
                  {
      ...
      /**
       * Notification that form is being destroyed.  This method now does nothing - when used in the form servce alone, this method
       * used to be able to tidy up some internal data as the form service works by creating and destroying a new object every time
       * the form (aka Edit Metadata dialogue) is opened.  The upload dialogue, however, simple shows and hides itself and expects a single
       * instance of the object picker to survice through the lifespan of the page.
       *
      ...
      _loadSelectedItems: function ObjectFinder__loadSelectedItems(useOptions)
      {
         ...
         {
        	if (this.selectedItems == null) //Was hitting null references in isSelectable code which was breaking control
        	{
        		this.selectedItems = {};
        	}
        	
        	if (this.options.currentValue=="" && this.options.displayMode == "items") //if no items selected explicitly blank out list of items
            {
        		this.selectedItems = {}
      		    Dom.get(this.id + "-currentValueDisplay").innerHTML ="<span class='none'>(none)</span>"
            }
   ...
   YAHOO.extend(Alfresco.ObjectRenderer, Alfresco.component.Base,
   {
      ...
      onCreateNewItem: function ObjectRenderer_onCreateNewItem()
      {
         ...
         if (elInput)
         {
            ...
            itemName = YAHOO.lang.trim(itemName.replace(/[^\w\s+#\-.]/g, "")).replace(/[\s|_]+/g, "-");
      ...
      _generatePickerChildrenUrlParams: function ObjectRenderer__generatePickerChildrenUrlPath(searchTerm)
      {
    	 ...
    	 if (this.options.itemFamily=="siteTags")
    	 {
    		 var url=window.location.href;
    		 itemType=url.substring(url.indexOf("/share/page/site/")+17, url.indexOf("/", url.indexOf("/share/page/site/")+17));
         }
    	 var params = "?selectableType=" + itemType + "&searchTerm=" + encodeURIComponent(searchTerm) + 
