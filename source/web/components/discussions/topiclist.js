      ...
      * Total number of topics in the current view (across all pages)
      ...
      onReady: function DiscussionsTopicList_onReady()
      {
         ...
         var generateTopicActions = function(me, data, tagName)
         {
            ...
            {
               html += '<' + tagName + ' class="onUndeleteTopic"><a href="#" class="topic-action-link-' + tagName + '">' + me._msg("action.undelete") + '</a></' + tagName + '>';
            }
            else
            {
   	         //If we're not in a deleted items site, show the mark for delete or remove delete mark buttons as appropriate
   	         if (data.archiveDue) {
   	             html += '<' + tagName + ' class="onRemoveDeleteMarkTopic"><a href="#" class="topic-action-link-' + tagName + '">' + me._msg("action.removeDeleteMark") + '</a></' + tagName + '>';
   	         } else {
   	        	 html += '<' + tagName + ' class="onMarkForDeleteTopic"><a href="#" class="topic-action-link-' + tagName + '">' + me._msg("action.markForDelete") + '</a></' + tagName + '>';
   	         }

   	         if (me.reasons.length > 0) {
    	   	    if (data.perishDue) {
	 	   	    	html += '<' + tagName + ' class="onRemovePerishableMarkTopic"><a href="#" class="topic-action-link-' + tagName + '">' + me._msg("action.removePerishableMark") + '</a></' + tagName + '>';
	 	        } else {
	 	        	html += '<' + tagName + ' class="onMarkAsPerishableTopic"><a href="#" class="topic-action-link-' + tagName + '">' + me._msg("action.markAsPerishable") + '</a></' + tagName + '>';
	 	        }
   	         }
   	         
   	         if ((data.archiveDue || data.perishDue) && data.deletionAuthorisation=="deleter") {
   	           html += '<' + tagName + ' class="onDeleteNowTopic"><a href="#" class="topic-action-link-' + tagName + '">' + me._msg("action.deleteNow") + '</a></' + tagName + '>';
   	         }
            }
         ...
         var renderTopic = function DiscussionsTopicList_renderTopic(elCell, oRecord, oColumn, oData)
         {
            ...
            var authorLink = Alfresco.util.people.generateUserLinkWithPresence(data.author, data.authorPresence);
            ...
            if (!me.options.simpleView)
            {
               ...
               html +='<div class="eslPMDiscussion'+data.eslPM.replace(/ /g,'')+'"> <span class="eslRenderNod">'+data.eslNod+'</span> <span class="eslRenderPM">'+data.eslPM+'</span> <span class="eslRenderAtomal">'+data.eslAtomal+'</span> <span class="eslRenderFreeForm">'+data.eslFreeFormCaveats+'</span> <span class="eslRenderEyes">'+data.eslEyes+'<br/>';
       ...
       */
      onMarkForDeleteTopic: function DiscussionsTopicList_onMarkForDeleteTopic(row)
      {
    	  var record = this.widgets.dataTable.getRecord(row);
          ...
          Alfresco.util.Ajax.request(
          {
               url : Alfresco.constants.PROXY_URI + "sv-theme/delete/markForDelete?nodeRef="+record.getData('nodeRef'),
               method: "POST",
               successCallback :
            {
               fn: markForDeleteOK,
       ...
       */
      onUndeleteTopic: function DiscussionsTopicList_onUndeleteTopic(row)
      {
    	  var record = this.widgets.dataTable.getRecord(row);
    	  
    	  var undeleteOK = function()
          {
    		alert("This discussion has been moved to its original location");
          ...
          Alfresco.util.Ajax.request(
          {
               url : Alfresco.constants.PROXY_URI + "sv-theme/delete/undelete?nodeRef="+record.getData('nodeRef'),
               method: "PUT",
               successCallback :
            {
               fn: undeleteOK,
       ...
       */
      onRemoveDeleteMarkTopic: function DiscussionsTopicList_onRemoveDeleteMarkTopic(row)
      {
    	  var record = this.widgets.dataTable.getRecord(row);
          ...
          Alfresco.util.Ajax.request(
          {
               url : Alfresco.constants.PROXY_URI + "sv-theme/delete/removeDeletionMark?nodeRef="+record.getData('nodeRef'),
               method: "PUT",
               successCallback :
            {
       ...
       */
      onDeleteNowTopic: function DiscussionsTopicList_onDeleteNowTopic(row)
      {
    	  var record = this.widgets.dataTable.getRecord(row);
          ...
          Alfresco.util.Ajax.request(
          {
               url : Alfresco.constants.PROXY_URI + "sv-theme/delete/archive?nodeRef="+record.getData('nodeRef'),
               method: "POST",
               successCallback :
            {
       ...
       */
      onMarkAsPerishableTopic: function DiscussionsTopicList_onMarkAsPerishableTopic(row)
      {
    	  var me = this;
          ...
          var handleSubmit = function() {
        	  if (me.markPerishableDialog.getData().perishable) {
        		  ...
        		  var record = me.widgets.dataTable.getRecord(row);
      	  
        		  var markAsPerishableOK = function() {
        			  self.location.href=self.location.href;
        		  }, markAsPerishableFail = function() {
        			  alert('An error occurred while attempting to mark this item as perishable.  If the error persists, please contact support');
        		  };
          	
        		  Alfresco.util.Ajax.request(
        				  {
        					  url : Alfresco.constants.PROXY_URI + "sv-theme/delete/markPerishable?nodeRef="+record.getData('nodeRef'),
      ...
      /**
       * Remove perishable mark on the given topic
       */
      onRemovePerishableMarkTopic: function DiscussionsTopicList_onRemovePerishableMarkTopic(row)
      {
    	  var record = this.widgets.dataTable.getRecord(row);
