...
*/
      ...
      /**
       * Mark Multiple Assets for Deletion
       * 
       * @method onActionMarkForDelete
       * @param assets {object} Object literal representing one or more file(s) or folder(s) to be actioned
       */
      onActionMarkForDelete: function DLTB_onActionMarkForDelete(assets) {
          var multipleAssets = [];
          var nodeRefParams = "";
  		  ...
  		  that.modules.actions.genericAction(
		  {
		     success:
		     {
		        message: that.msg("message.mark-many-for-delete.success"),
		        event: { name: "metadataRefresh" }
			 },
			 failure:
			 {
			    message: that.msg("message.mark-many-for-delete.failure")
			 },
			 webscript:
			 {
		        name: "/sv-theme/delete/markForDelete?{nodeRefParams}",
		        method: Alfresco.util.Ajax.POST
		     },
		     params:
		     {
		        nodeRefParams: nodeRefParams
		     },
		     config:
		     {
        ...
        for (i = 0, ii = assets.length; i < ii; i++)
        {
           ...
           nodeRefParams += ((i == 0 ? "" : "&") + "nodeRef=" + assets[i].nodeRef);
           includesFolder = includesFolder || assets[i].isFolder;
        }        

		if(includesFolder) {
			this._confirmManagedDeletionOperation(
					that.msg("message.mark-many-for-delete.folder.confirm-dialog.title"),
					that.msg("message.mark-many-for-delete.folder.confirm-dialog.text"),
					markForDelete)
	    } else {
		   markForDelete();
	    }
     ...
     /**
      * Mark Multiple Assets as Perishable
      * 
      * @method onActionMarkForDelete
      * @param assets {object} Object literal representing one or more file(s) or folder(s) to be actioned
      */
     onActionMarkAsPerishable: function DLTB_onActionMarkAsPerishable(assets) {
        var multipleAssets = [];
        var nodeRefParams = "";
        var that = this;
      		
   	  	var markPerishable = function(perishReason) {
  			
   	  	  that.modules.actions.genericAction(
		  {
		     success:
		     {
		        message: that.msg("message.mark-many-as-perishable.success"),
		        event: { name: "metadataRefresh" }
			 },
			 failure:
			 {
			    message: that.msg("message.mark-many-as-perishable.failure")
			 },
			 webscript:
			 {
		        name: "/sv-theme/delete/markPerishable?{nodeRefParams}",
		        method: Alfresco.util.Ajax.POST
		     },
		     params:
		     {
 		...
 		var handleSubmit = function() {
         	if (this.getData().perishable) {
         		var reason = this.getData().perishable;
         		
         		markPerishable(reason);
         	}
       ...
       for (i = 0, ii = assets.length; i < ii; i++)
       {
     	  // Skip if the asset is a folder or is already perishable
     	  if(assets[i].isFolder || assets[i].perishReason) {
     		  continue;
     	  }
          multipleAssets.push(assets[i].nodeRef);             
          nodeRefParams += ((i == 0 ? "" : "&") + "nodeRef=" + assets[i].nodeRef);
       ...
       var documentList = Alfresco.util.ComponentManager.findFirst("Alfresco.DocumentList");
       var markPerishableDialog = documentList.markPerishableDialog;
       
       if(!markPerishableDialog) {
    	   // This should never happen
    	   alert("ERROR: Could not find dialog");
       }
       
       markPerishableDialog.cfg.queueProperty("buttons",
      		 [
      		  { text: "Submit", handler: handleSubmit, isDefault: true },
                { text: "Cancel", handler: handleCancel }
               ]);

       markPerishableDialog.render();
       markPerishableDialog.show();
    },
    
    /**
     * Remove Perishable Mark from Multiple Assets
     * 
     * @method onActionRemovePerishableMark
     * @param assets {object} Object literal representing one or more file(s) or folder(s) to be actioned
     */
    onActionRemovePerishableMark: function DLTB_onActionRemovePerishableMark(assets) {
       var multipleAssets = [];
       var nodeRefParams = "";
       var that = this;
  	  	  ...
  	  	  that.modules.actions.genericAction(
		  {
		     success:
		     {
		        message: that.msg("message.remove-many-perishable-mark.success"),
		        event: { name: "metadataRefresh" }
			 },
			 failure:
			 {
			    message: that.msg("message.remove-many-perishable-mark.failure")
			 },
			 webscript:
			 {
		        name: "/sv-theme/delete/markPerishable?{nodeRefParams}",
      ...
      for (i = 0, ii = assets.length; i < ii; i++)
      {
    	  // Skip if the asset is a folder or is not perishable
    	  if(assets[i].isFolder || !assets[i].perishReason) {
    		  continue;
    	  }
      ...
      /**
       * Remove Deletion Mark from Multiple Assets
       * 
       * @method onActionRemoveDeleteMark
       * @param assets {object} Object literal representing one or more file(s) or folder(s) to be actioned
       */
      onActionRemoveDeleteMark: function DLTB_onActionRemoveDeleteMark(assets) {
          var multipleAssets = [];
          var nodeRefParams = "";
          var includesFolder = false;
          var that = this;
  		  ...
  		  that.modules.actions.genericAction(
          {
             success:
             {
                message: that.msg("message.remove-many-delete-mark.success"),
                event: { name: "metadataRefresh" }
             },
             failure:
             {
                message: that.msg("message.remove-many-delete-mark.failure")
             },
             webscript:
             {
                name: "/sv-theme/delete/removeDeletionMark?{nodeRefParams}",
  		...
  		if(includesFolder) {
  			this._confirmManagedDeletionOperation(
  					that.msg("message.remove-many-delete-mark.folder.confirm-dialog.title"),
  					that.msg("message.remove-many-delete-mark.folder.confirm-dialog.text"),
  					removeDeletionMark)
  	    } else {
  	    	removeDeletionMark();
  	    }
      ...
      /**
       * Delete Multiple Assets Now
       * 
       * @method onActionDeleteNow
       * @param assets {object} Object literal representing one or more file(s) or folder(s) to be actioned
       */
      onActionDeleteNow: function DLTB_onActionDeleteNow(assets) {
          var multipleAssets = [];
          var nodeRefParams = "";
          var includesFolder = false;
          var that = this;
  		  ...
  		  that.modules.actions.genericAction(
          {
             success:
             {
                message: that.msg("message.delete-many-now.success"),
                event: { name: "metadataRefresh" }
             },
             failure:
             {
                message: that.msg("message.delete-many-now.failure")
             },
             webscript:
             {
                name: "sv-theme/delete/archive?{nodeRefParams}",
                method: Alfresco.util.Ajax.POST
             },
             params:
             {
    		...
    		if(includesFolder) {
    			this._confirmManagedDeletionOperation(
    					that.msg("message.delete-many-now.folder.confirm-dialog.title"),
    					that.msg("message.delete-many-now.folder.confirm-dialog.text"),
    					deleteNow)
    	    } else {
    	    	deleteNow();
    	    }
      ...
      /**
       * Undelete Multiple Assets
       * 
       * @method onActionDeleteNow
       * @param assets {object} Object literal representing one or more file(s) or folder(s) to be actioned
       */
      onActionUndoDelete: function DL_onActionUndoDelete(assets)
      {
    	  var multipleAssets = [];
          var nodeRefParams = "";
          var includesFolder = false;
  		  ...
  		  that.modules.actions.genericAction(
          {
             success:
             {
                message: that.msg("message.undelete-many.success"),
                event: { name: "metadataRefresh" }
             },
             failure:
             {
                message: that.msg("message.undelete-many.failure")
             },
             webscript:
             {
                name: "sv-theme/delete/undelete?{nodeRefParams}",
                method: Alfresco.util.Ajax.PUT
             },
             params:
             {
  		...
  		if(includesFolder) {
  			this._confirmManagedDeletionOperation(
  					that.msg("message.undelete-many.folder.confirm-dialog.title"),
  					that.msg("message.undelete-many.folder.confirm-dialog.text"),
  					deleteNow)
  	    } else {
  	    	deleteNow();
  	    }
      ...
      onHideNavBar: function DLTB_onHideNavBar(e, p_obj)
      {
         ...
         YAHOO.Bubbling.fire("onHideNavBar", {
             visible: !this.options.hideNavBar
         });
