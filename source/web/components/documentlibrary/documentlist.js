...
(function()
{
   ...
   var $html = Alfresco.util.encodeHTML,
      ...
      $userProfileWithPresence = Alfresco.thirdparty.presence.getPresenceIndicatorHtml,
   ...
   var PREFERENCES_DOCLIST = "org.alfresco.share.documentList",
      ...
      PREF_SORT_DIRECTION = PREFERENCES_DOCLIST +".sortDirection",
   ...
   YAHOO.lang.augmentObject(Alfresco.DocumentList.prototype,
   {
      ...
      onReady: function DL_onReady()
      {
         ...
         this.widgets.sortDirection = Alfresco.util.createYUIButton(this, "sortDirection-button", this.onSortDirection);
         if (this.widgets.sortDirection !== null)
         {
            if (!this.widgets.sortDirection)
            {
               Dom.addClass(this.widgets.sortAscending.get("element"), "sort-descending");
            }
            this.sortControls.push(this.widgets.sortDirection);
         }
         ...
         if (this.widgets.sortField !== null) {
        	 ...
        	 this.widgets.sortField.set("label", "Name");
         }
      ...
      fnRenderCellDescription: function DL_fnRenderCellDescription()
      {
         ...
         return function DL_renderCellDescription(elCell, oRecord, oColumn, oData)
         {
            ...
            var record = oRecord.getData(),
               type = record.type,
               uid = record.nodeRef.replace("workspace://SpacesStore/", ""), 
               isLink = record.isLink,
               title = "",
            ...
            if (type == "folder")
            {
                ...
                * Folders
                */
               if (record.folderArchiveDue)
               {
                 desc +='<div class="deleteMarkWrapper">';
             	 desc +='<div id="DeletionMark-'+uid+'" class="markedForDeletion">Contents marked for deletion <span class="markMeta">by <a href="/share/page/user/'+record.folderDeletedBy+'/profile">'+record.folderDeletedBy+'</a></span></div>';              
             	 desc +='</div>';  
               }
               ...
               if (scope.options.simpleView)
               {
                  ...
                  desc += '<span class="item-simple"><em>' + scope.msg("details.by") + '</em> ' + $userProfileWithPresence(record.modifiedByUser, record.modifiedBy, record.modifiedByUserPresence) + '</span></div>';
               }
               else
               {
                  /**
                  ...
                  desc += '<span class="item"><em>' + scope.msg("details.modified.by") + '</em> ' + $userProfileWithPresence(record.modifiedByUser, record.modifiedBy, record.modifiedByUserPresence) + '</span></div>';
            ...
            else
            {
                ...
                * Documents and Links
                */
               desc +='<div class="eslPMDocLibList'+record.eslPM.replace(/ /g,'')+'"> <span class="eslRenderNod">'+record.eslNod+'</span> <span class="eslRenderPM">'+record.eslPM+'</span> <span class="eslRenderAtomal">'+record.eslAtomal+'</span> <span class="eslRenderFreeForm">'+record.eslFreeformCaveats+'</span> <span class="eslRenderEyes">'+record.eslEyes+'</span><br/>';
               desc +='<span class="eslRenderClosed">'+record.eslClosedMarkings+' </span> <span class="eslRenderOrganisations">'+record.eslOrganisations+'</span><span class="eslRenderOpen">'+record.eslOpenMarkings+'</span> </div>';
               
               desc +='<div class="deleteMarkWrapper">';
               
               if (record.archivalStatus && record.archivalStatus.archivalDue) {
            	   desc += "<div class='deleteDate'>Due for deletion on <span class='date'>"+record.archivalStatus.archivalDue+"</span></div>";
               }
               
               if (record.archiveDue)
               {
            	 desc +='<div id="DeletionMark-'+uid+'" class="markedForDeletion">Marked for deletion <span class="markMeta">by <a href="/share/page/user/'+record.deletedBy+'/profile">'+record.deletedBy+'</a></span></div>';
               }
               
               if (record.perishDue)
               {
            	 var perishLabel = scope.msg("label.markedAsPerishable", record.perishTitle, record.perishRequestedBy);
            	 desc +='<div id="PerishableMark-'+uid+'" class="markedAsPerishable">'+perishLabel+' <span class="markMeta">by <a href="/share/page/user/'+record.perishRequestedBy+'/profile">'+record.perishRequestedBy+'</a></span></div>';
               }   
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     desc += '<span class="item"><em>' + scope.msg("details.modified.by") + '</em> ' + $userProfileWithPresence(record.modifiedByUser, record.modifiedBy, record.modifiedByUserPresence) + '</span>';
      ...
      onSortDirection: function (e, p_obj) {
          this.options.sortDirection = !this.options.sortDirection;
          this.services.preferences.set(PREF_SORT_DIRECTION, this.options.sortDirection);
          
          if (this.options.sortDirection)
          {
             Dom.removeClass(this.widgets.sortDirection.get("element"), "sort-descending");
          }
          else
          {
             Dom.addClass(this.widgets.sortDirection.get("element"), "sort-descending");
          }
      ...
      onSortField: function(sType, aArgs, p_obj) {
	          ...
	          this.services.preferences.set(PREF_SORT_FIELD, this.options.sortField, {
	             successCallback:
	             {
	                fn: function()
	                {
	                   ...
	                   {
	                      me.options.onSortDirection = (title === "false");
	                      // onSortDirection will fire the metadatRefresh event
	                      me.onSortDirection();
	                   }
	                   else
	                   {
      ...
      onEventHighlightRow: function DL_onEventHighlightRow(oArgs)
      {
         ...
         //Add onlick handlers to download links        
         var downloadLinks=YAHOO.util.Selector.query('div.onActionDownload a');
         for (var i=0; i < downloadLinks.length; i++) {
        	 if (downloadLinks[i].href=="{downloadUrl}" || downloadLinks[i].href=="%7BdownloadUrl%7D") {
        		 continue;
        	 }
        	 downloadLinks[i].onclick=function() {
        		 if (this.href.match(/&d$/)) {
        			 var documentDetailsUrl=this.href;
        			 var parentId=this.parentNode.parentNode.id;
        			 this.href=YAHOO.util.Selector.query('div#'+parentId+' div.onActionView a')[0].href+'?a=true';
        			 YAHOO.lang.later(250,this, function(){this.href=documentDetailsUrl;}, false);
        		 }
        		 return true;
             }
         }
      ...
      _confirmManagedDeletionOperation: function(title, text, action) {
		Alfresco.util.PopupManager.displayPrompt({
			title: title,
			text: text,
			noEscape: true,
      ...
      onActionMarkForDelete: function DL_onActionMarkForDelete(file)
      {
		...
		var markForDelete = function() {
			...
			if(file.isFolder) {
				successMessage = that.msg("message.mark-folder-for-delete.success", file.displayName);
				failureMessage = that.msg("message.mark-folder-for-delete.failure", file.displayName);
			} else {
				successMessage = that.msg("message.mark-for-delete.success", file.displayName);
				failureMessage = that.msg("message.mark-for-delete.failure", file.displayName);
			}
    		...
    		if(file.isFolder) {
    			this._confirmManagedDeletionOperation(
					this.msg("message.mark-many-for-delete.folder.confirm-dialog.title", file.displayName),
					this.msg("message.mark-many-for-delete.folder.confirm-dialog.text", file.displayName),
					markForDelete
   				);
		   } else {
			   markForDelete();
      ...
      onActionRemoveDeleteMark: function DL_onActionRemoveDeleteMark(file)
      {
    		...
    		var removeDeletionMark = function() {
    			var successMessage, failureMessage;
    			
    			if(file.isFolder) {
    				successMessage = that.msg("message.remove-folder-delete-mark.success", file.displayName);
    				failureMessage = that.msg("message.remove-folder-delete-mark.failure", file.displayName);
    			} else {
    				successMessage = that.msg("message.remove-delete-mark.success", file.displayName);
    				failureMessage = that.msg("message.remove-delete-mark.failure", file.displayName);
    			}
    		...
    		if(file.isFolder) {
    			this._confirmManagedDeletionOperation(
					this.msg("message.remove-delete-mark.folder.confirm-dialog.title", file.displayName),
					this.msg("message.remove-delete-mark.folder.confirm-dialog.text", file.displayName),
					removeDeletionMark
   				);
		   } else {
			   removeDeletionMark();
    	  ...
    	  var markAsPerishable = function() {
            ...
            var handleSubmit = function() {
         	if (me.markPerishableDialog.getData().perishable) {
    			...
    			if(file.isFolder) {
    				successMessage = me.msg("message.mark-folder-as-perishable.success");
    				failureMessage = me.msg("message.mark-folder-as-perishable.failure");
    			} else {
    				successMessage = me.msg("message.mark-as-perishable.success");
    				failureMessage = me.msg("message.mark-as-perishable.failure");
    			}
            ...
            me.markPerishableDialog.cfg.queueProperty("buttons",
           		 [
           		  { text: "Submit", handler: handleSubmit, isDefault: true },
                     { text: "Cancel", handler: handleCancel }
                    ]);

            me.markPerishableDialog.render();
            Alfresco.thirdparty.perishability.validateDialog();
	       	me.markPerishableDialog.show();
    	  };

    		if(file.isFolder) {
    			this._confirmManagedDeletionOperation(
					this.msg("message.mark-many-as-perishable.folder.confirm-dialog.title", file.displayName),
					this.msg("message.mark-many-as-perishable.folder.confirm-dialog.text", file.displayName),
					markAsPerishable
   				);
		   } else {
			   markAsPerishable();
      ...
      onActionRemovePerishableMark: function DL_onActionRemovePerishableMark(file)
      {
    		...
    		var removePerishableMark = function() {
    			var successMessage, failureMessage;
    			
    			if(file.isFolder) {
    				successMessage = that.msg("message.remove-folder-perishable-mark.success", file.displayName);
    				failureMessage = that.msg("message.remove-folder-perishable-mark.failure", file.displayName);
    			} else {
    				successMessage = that.msg("message.remove-perishable-mark.success", file.displayName);
    				failureMessage = that.msg("message.remove-perishable-mark.failure", file.displayName);
    			}
    		...
    		if(file.isFolder) {
    			this._confirmManagedDeletionOperation(
					this.msg("message.remove-perishable-mark.folder.confirm-dialog.title", file.displayName),
					this.msg("message.remove-perishable-mark.folder.confirm-dialog.text", file.displayName),
					removePerishableMark
   				);
		   } else {
			   removePerishableMark();
		   }
      ...
      onActionUndoDelete: function DL_onActionUndoDelete(file)
      {
		...
		var undoDelete = function() {
			...
			if(file.isFolder) {
				successMessage = that.msg("message.undelete-folder.success", file.displayName);
				failureMessage = that.msg("message.undelete-folder.failure", file.displayName);
			} else {
				successMessage = that.msg("message.undelete.success", file.displayName);
				failureMessage = that.msg("message.undelete.failure", file.displayName);
			}
		...
		if(file.isFolder) {
			this._confirmManagedDeletionOperation(
				this.msg("message.undelete.folder.confirm-dialog.title", file.displayName),
				this.msg("message.undelete.folder.confirm-dialog.text", file.displayName),
				undoDelete
				);
	   } else {
		   undoDelete();
      ...
      onActionDeleteNow: function DL_onActionUndoDelete(file)
      {
    		...
    		var deleteNow = function() {
    			...
    			if(file.isFolder) {
    				successMessage = that.msg("message.delete-now-folder.success", file.displayName);
    				failureMessage = that.msg("message.delete-now-folder.failure", file.displayName);
    			} else {
    				successMessage = that.msg("message.delete-now.success", file.displayName);
    				failureMessage = that.msg("message.delete-now.failure", file.displayName);
    			}
    		...
    		if(file.isFolder) {
    			this._confirmManagedDeletionOperation(
    				this.msg("message.delete-now.folder.confirm-dialog.title", file.displayName),
    				this.msg("message.delete-now.folder.confirm-dialog.text", file.displayName),
    				deleteNow
    				);
    	   } else {
    		   deleteNow();
         ...
         var failureHandler = function DL__uDL_failureHandler(sRequest, oResponse)
         {
            ...
            else
            {
               try
               {
                  ...
                  {
                	  // 404 on this folder of the doclib? Switch up a level.
                      Alfresco.util.PopupManager.displayMessage({"text": this.msg("message.folder-not-found")});
                      var url = window.location.href;
                      var path = url.match(/filter=path%7C[^&?]+/)[0];
                      window.location.href = url.replace(/filter=path%7C[^&?]+/, path.substring(0, path.lastIndexOf('/')));
                  }
                  else
                  {
      ...
      _buildDocListParams: function DL__buildDocListParams(p_obj)
      {
         ...
         // Sort parameters
         params += "&sortAsc=" + this.options.sortDirection + "&sortField=" + encodeURIComponent(this.options.sortField);
